...
double vy[N], yold[N];
double Fx[N];
double Fy[N], xout[N/2],yout[N/2],dxout[N/2],dyout[N/2];
...
*/
    ...
    for (a=0;a<N/2;a++)
    {
       ...
       dxout[a]=x[b]-x[a];
       dyout[a]=y[b]-y[a];
        if (dxout[a] > sizex / 2) dxout[a] = dxout[a] - sizex;
        else if (dxout[a] < -sizex / 2) dxout[a] = dxout[a] + sizex;

        if (dyout[a] > sizey / 2) dyout[a] = dyout[a] - sizey;
        else if (dyout[a] < -sizey / 2) dyout[a] = dyout[a] + sizey;

    }

    for (a=0;a<N/2;a+=2) {
        fprintf(fp, "%lf %lf %lf %lf\n", xout[a], yout[a], dxout[a], dyout[a]);
    }
